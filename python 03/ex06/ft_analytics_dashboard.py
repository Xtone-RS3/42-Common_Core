if __name__ == "__main__":
    ...
    print(f"High scorers (>2000): {[player for player in everything['players'] if everything['players'][player]['total_score'] > 2000]}")
    print(f"Scores doubled: {[everything['players'][player]['total_score'] * 2 for player in everything['players']]}")
    print(f"Casuals: {[player for player in everything['players'] if everything['players'][player]['favorite_mode'] == 'casual']}")
    print()
    print("=== Dict Comprehension Examples ===")
    scores = {player: everything['players'][player]['total_score'] for player in everything['players']}
    print(f"Player scores: {scores}")
    score_categories = {8000: "high", 4000: "medium", 0: "low"}
    score_breakdown = {cat: sum(1 for s in scores.values() if s > threshold) for threshold, cat in score_categories.items()}
    print(f"Score categories: {score_breakdown}")
    achi_count = {player: everything['players'][player]['achievements_count'] for player in everything['players']}
    ...
    print(f"Total unique achievements: {sum(1 for achievemnt in everything['achievements'])}")
    print(f"Average score per session: {sum(sesh['score'] for sesh in everything['sessions']) / sum(1 for sesh in everything['sessions']):0.2f}")
    """
