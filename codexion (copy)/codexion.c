...
void	eat(t_coders *coders)
{
	...
	if (coders->config->number_of_coders == 1) //philo->num_of_philos
	{
		ft_usleep(coders->time_to_burnout); //coders->config->base_burnout_time
	...
	ft_usleep(coders->time_to_compile); //->time_to_eat // coders->config->base_compile_time
	coders->compiling = 0;
...
int	main(int argc, char **argv)
{
	...
	// printf("real config: %p\ncoders config: %p\n", &config, &coders[0].config);
	// printf("number main: %d\n", coders->config->number_of_coders);
	...
	// 	pthread_mutex_init(&program.write_lock, NULL);
	// 	pthread_mutex_init(&program.coders[n_coders].r_dongle, NULL);
	// 	pthread_mutex_init(&program.coders[n_coders].l_dongle, NULL);
	// 	pthread_create(&program.coders[n_coders].thread, NULL, foo, &program.coders[n_coders]);
	// 	// printf("number: %d\n", program.coders[n_coders].id);
		
	// 	n_coders += 1;
	// }

	// if (pthread_equal(program.coders[0].thread, program.coders[1].thread))
	// {
	...
	// while (n_coders != atoi(argv[1]))
	// {
	// 	pthread_join(program.coders[n_coders].thread, NULL);
	// 	n_coders += 1;
	// }
	...
	// while (n_coders != atoi(argv[1]))
	// {
	// 	pthread_mutex_destroy(&program.coders[n_coders].r_dongle);
	// 	pthread_mutex_destroy(&program.coders[n_coders].l_dongle);
	// 	n_coders += 1;
	// }
	// pthread_mutex_destroy(&program.write_lock);
	free(program.coders);
