...
class CardGenerator:
    ...
    def __init__(self) -> None:
        ...
        self._creatures = [
            {"name": "Fire Dragon", "cost": 5, "rarity": "Legendary", "attack": 7, "health": 5},  # noqa
            {"name": "Goblin Warrior", "cost": 2, "rarity": "Common", "attack": 2, "health": 1},  # noqa
            {"name": "Ice Wizard", "cost": 4, "rarity": "Rare", "attack": 3, "health": 4},  # noqa
            {"name": "Lightning Elemental", "cost": 3, "rarity": "Uncommon", "attack": 4, "health": 2},  # noqa
            {"name": "Stone Golem", "cost": 6, "rarity": "Rare", "attack": 5, "health": 8},  # noqa
            {"name": "Shadow Assassin", "cost": 3, "rarity": "Uncommon", "attack": 5, "health": 2},  # noqa
            {"name": "Healing Angel", "cost": 4, "rarity": "Rare", "attack": 2, "health": 6},  # noqa
            {"name": "Forest Sprite", "cost": 1, "rarity": "Common", "attack": 1, "health": 1},  # noqa
        ]

        self._spells = [
            {"name": "Lightning Bolt", "cost": 3, "rarity": "Common", "effect_type": "damage"},  # noqa
            {"name": "Healing Potion", "cost": 2, "rarity": "Common", "effect_type": "heal"},  # noqa
            {"name": "Fireball", "cost": 4, "rarity": "Uncommon", "effect_type": "damage"},  # noqa
            {"name": "Shield Spell", "cost": 1, "rarity": "Common", "effect_type": "buff"},  # noqa
            {"name": "Meteor", "cost": 8, "rarity": "Legendary", "effect_type": "damage"},  # noqa
            {"name": "Ice Shard", "cost": 2, "rarity": "Common", "effect_type": "damage"},  # noqa
            {"name": "Divine Light", "cost": 5, "rarity": "Rare", "effect_type": "heal"},  # noqa
            {"name": "Magic Missile", "cost": 1, "rarity": "Common", "effect_type": "damage"},  # noqa
        ]

        self._artifacts = [
            {"name": "Mana Crystal", "cost": 2, "rarity": "Common", "durability": 5, "effect": "Permanent: +1 mana per turn"},  # noqa
            {"name": "Sword of Power", "cost": 3, "rarity": "Uncommon", "durability": 3, "effect": "Permanent: +2 attack to equipped creature"},  # noqa
            {"name": "Ring of Wisdom", "cost": 4, "rarity": "Rare", "durability": 4, "effect": "Permanent: Draw an extra card each turn"},  # noqa
            {"name": "Shield of Defense", "cost": 5, "rarity": "Rare", "durability": 6, "effect": "Permanent: +3 health to all friendly creatures"},  # noqa
            {"name": "Crown of Kings", "cost": 7, "rarity": "Legendary", "durability": 8, "effect": "Permanent: +1 cost reduction to all cards"},  # noqa
            {"name": "Boots of Speed", "cost": 2, "rarity": "Uncommon", "durability": 2, "effect": "Permanent: Cards cost 1 less mana"},  # noqa
            {"name": "Cloak of Shadows", "cost": 3, "rarity": "Uncommon", "durability": 3, "effect": "Permanent: Creatures have stealth"},  # noqa
            {"name": "Staff of Elements", "cost": 6, "rarity": "Legendary", "durability": 7, "effect": "Permanent: +1 spell damage"},  # noqa
        ]
    ...
    def get_cards_by_rarity(self, rarity: str) -> Dict[str, List[Dict[str, Any]]]:  # noqa
        """
        Get all cards of a specific rarity.
          # noqa
        Args:
            rarity: The rarity to filter by ("Common", "Uncommon", "Rare", "Legendary")  # noqa
            
        Returns:
            Dictionary with card types as keys and lists of matching cards as values  # noqa
        """
        ...
        return result
      # noqa
    def get_cards_by_cost(self, max_cost: int) -> Dict[str, List[Dict[str, Any]]]:  # noqa
        """
        ...
        return result
      # noqa
    def generate_random_deck(self, deck_size: int = 10) -> List[Dict[str, Any]]:  # noqa
        """
    ...
    def list_available_cards(self) -> None:
        ...
        print("Creatures:")
        for creature in self._creatures:
            print(f"  - {creature['name']} ({creature['cost']} mana, {creature['rarity']})")  # noqa

        print("\nSpells:")
        for spell in self._spells:
            print(f"  - {spell['name']} ({spell['cost']} mana, {spell['rarity']})")  # noqa

        print("\nArtifacts:")
        for artifact in self._artifacts:
            print(f"  - {artifact['name']} ({artifact['cost']} mana, {artifact['rarity']})")  # noqa
...
def main() -> None:
    ...
    for card_type, cards in legendary_cards.items():
        if cards:
            print(f"{card_type.capitalize()}: {[card['name'] for card in cards]}")  # noqa
    ...
    print(f"\nRandom Deck (5 cards): {[card['name'] for card in generator.generate_random_deck(5)]}")  # noqa
