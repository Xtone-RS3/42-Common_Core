...
void	draw_map(t_game *g)
{
	...
	while (g->map[y])
	{
		...
		{
			if (g->map[y][x] == '1')
				mlx_put_image_to_window(g->mlx, g->win, g->img_wall, x * g->t_s, y * g->t_s);
			else
				mlx_put_image_to_window(g->mlx, g->win, g->img_floor, x * g->t_s, y * g->t_s);
			if (g->map[y][x] == 'P')
				mlx_put_image_to_window(g->mlx, g->win, g->img_player, x * g->t_s, y * g->t_s);
			else if (g->map[y][x] == 'C')
				mlx_put_image_to_window(g->mlx, g->win, g->img_collectible, x * g->t_s, y * g->t_s);
			else if (g->map[y][x] == 'E' && g->score == g->max_score)
				mlx_put_image_to_window(g->mlx, g->win, g->img_exit_open, x * g->t_s, y * g->t_s);
			else if (g->map[y][x] == 'E')
				mlx_put_image_to_window(g->mlx, g->win, g->img_exit, x * g->t_s, y * g->t_s);
			x++;
		}
		y++;
	}
...
void	map_assign(int rows, t_game *g, char *to_open)
{
	...
	while (i < rows)
	{
		...
		g->map[i] = malloc(sizeof(char *) * (ft_strlen(line) + 1));
		if (!g->map[i])
			free(g->map);
		g->map[i][ft_strlen(line)] = '\0';
		while (line[j])
		{
			g->map[i][j] = line[j];
			j++;
...
void	map_open_and_row(int argc, char **argv, t_game *game)
{
	...
	rows = 0;
	if (argc > 2)
		ft_printf("put map.ber in you fucking idiot");
	else if (argc == 2)
