...
def gere_key(key, xvar):
    if key == 49:
        ...
        xvar.set_maze(lol)
        lol.maze_carver(entry[0], entry[1], xvar)
...
class MazeGenerator(object):  # this need to be in a standalone module
    ...
    def prim_carver(self, start_x, start_y):
        # 1. Marcar a célula inicial (start_x, start_y) como visitada no labirinto.
        self.maze[(start_x, start_y)]["visited"] = True
        # 2. Criar uma lista vazia para guardar as paredes fronteiriças.
        walls = []
        # 3. Encontrar as paredes vizinhas da célula inicial e adicioná-las à lista.
        neighbours = self.find_neighbours(start_x, start_y)
        if neighbours:
            for neighbour in neighbours:
                walls.append(neighbour)
        # 4. Iniciar um loop que corre enquanto a lista de paredes tiver elementos:
        while walls:
            # a. Escolher uma parede aleatória da lista.
            wall_data = random.choice(walls)
            (next_x, next_y) = wall_data[0]
            r_wall = wall_data[1]
            # d. Remover a parede da lista imediatamente (resolve o tal ciclo infinito!)
            walls.remove(wall_data)
            # b e c. Confirmar se a célula candidata AINDA NÃO foi visitada:
            if self.maze[(next_x, next_y)]["visited"] == False:
                # i. "Derrubar" a parede entre elas (mudar o valor de 1 para 0).
                if r_wall.startswith("s"):
                    origin_x = next_x
                    origin_y = next_y - 1
                    self.maze[(origin_x, origin_y)]["south"] = 0
                    self.maze[(next_x, next_y)]["north"] = 0
                elif r_wall.startswith("n"):
                    origin_x = next_x
                    origin_y = next_y + 1
                    self.maze[(origin_x, origin_y)]["north"] = 0
                    self.maze[(next_x, next_y)]["south"] = 0
                elif r_wall.startswith("e"):
                    origin_x = next_x - 1
                    origin_y = next_y
                    self.maze[(origin_x, origin_y)]["east"] = 0
                    self.maze[(next_x, next_y)]["west"] = 0
                elif r_wall.startswith("w"):
                    origin_x = next_x + 1
                    origin_y = next_y
                    self.maze[(origin_x, origin_y)]["west"] = 0
                    self.maze[(next_x, next_y)]["east"] = 0
                # ii. Marcar a célula recém-descoberta como visitada.
                self.maze[(next_x, next_y)]["visited"] = True
                # iii. Encontrar as paredes vizinhas dessa nova célula e adicioná-las à lista.
                new_neighbours = self.find_neighbours(next_x, next_y)
                if new_neighbours:
                    walls.extend(new_neighbours)
    ...
    def maze_carver(self, x, y, xvar):  # depth-first
        """
        ...
        while self.visit_count < self.size:
            ...
            if next_possible_cell is not None:
                ...
                if wall.startswith("s"):
                    ...
                    self.maze[(next_x, next_y)]["north"] = 0
                    draw_south_wall(xvar, x, y, self.block_size, 0xFF000000)
                    draw_north_wall(xvar, next_x, next_y, self.block_size, 0xFF000000)
                elif wall.startswith("n"):
                    self.maze[(x, y)]["north"] = 0
                    self.maze[(next_x, next_y)]["south"] = 0
                    draw_north_wall(xvar, x, y, self.block_size, 0xFF000000)
                    draw_south_wall(xvar, next_x, next_y, self.block_size, 0xFF000000)
                elif wall.startswith("e"):
                    self.maze[(x, y)]["east"] = 0
                    self.maze[(next_x, next_y)]["west"] = 0
                    draw_east_wall(xvar, x, y, self.block_size, 0xFF000000)
                    draw_west_wall(xvar, next_x, next_y, self.block_size, 0xFF000000)
                elif wall.startswith("w"):
                    self.maze[(x, y)]["west"] = 0
                    self.maze[(next_x, next_y)]["east"] = 0
                    draw_west_wall(xvar, x, y, self.block_size, 0xFF000000)
                    draw_east_wall(xvar, next_x, next_y, self.block_size, 0xFF000000)
                self.maze[(next_x, next_y)]["visited"] = True
                x = next_x
                y = next_y
                self.visit_count += 1
            ...
            if False not in [self.maze[cell]["visited"] for cell in self.maze]:
                print(len(self.hasVisited))
                return
            # elif next_possible_cell is None and len(self.hasVisited) == 0:
            #     if False in [self.maze[cell]["visited"] for cell in self.maze]:
            #         print("false")
            #     # print([self.maze[cell]["visited"] for cell in self.maze])
            #     return
...
Generating random maze with default values...\n")
    ...
    xvar.mlx.mlx_loop_hook(xvar.mlx_ptr, render_next_frame, xvar)
